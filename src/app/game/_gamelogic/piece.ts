...
export class King extends Piece
{
    ...
    private static getPossibleCaptures(from: Square, color: Color, board: Board, reachable: Square[]): BoardMove[]
    {
        return reachable.filter(square => board.at(square)?.color !== color) // Piece of other color on target square -> can capture
                        .map(square=> new BoardMove(from, square, true))
    }
    private static getPossibleCastles(from: Square, color: Color, board: Board): BoardMove[]
    {
        let retArr: BoardMove[] = []
...
export class Pawn extends Piece
{
    ...
    possibleMoves(from: Square, color: Color, board: Board): BoardMove[]
    {
        ...
        retArr = retArr.concat(Pawn.getPossibleCaptures(from, color, board));
        return retArr;
    }

    private static getPossibleSimpleMoves(from: Square, color: Color, board: Board): BoardMove[]
    {
        let retArr: BoardMove[] = [];
        let [aheadSquare, doubleMoveSquare] = Pawn.reachableMoveSquares(from, color);
    ...
    private static getPossibleCaptures(from: Square, color: Color, board: Board): BoardMove[]
    {
        let retArr: BoardMove[] = [];
        let [leftCaptureSquare, rightCaptureSquare] = Pawn.reachableCaptureSquares(from, color);
     ...
     */
    private static reachableMoveSquares(from: Square, color: Color): [Square | undefined, Square | undefined]
    {
        if (color === Color.WHITE)
        {
            return [from.above(), from.above()?.above()]
        }
        return [from.below(), from.below()?.below()]
    }
    private static reachableCaptureSquares(from: Square, color: Color): [Square | undefined, Square | undefined]
    {
        if (color === Color.WHITE)
        {
            return [from.topLeft(), from.topRight()]
        }
        return [from.bottomLeft(), from.bottomRight()]
    }
}
